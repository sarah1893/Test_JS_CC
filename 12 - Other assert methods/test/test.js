...
describe('Checkpoint 1', () => {
  it('asserts that `assert.notEqual` is in it block', () => {
    ...
    let failureMessage = 'Call `assert.notEqual` in the `it` block.';
    assert.isOk(isMatch, failureMessage);

    // assert that two digits are passed to assert.notEqual()
    let codeMatch = code.match(/assert\.\s*notEqual\s*\(\s*(\d)\s*,\s*(\d)\s*\)/);
    assert.isOk(codeMatch, 'Pass two numbers to `assert.notEqual`.');
